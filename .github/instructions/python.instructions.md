...
- **Declarative Style**: Prefer `match` statements for complex conditional logic and multi-branch cases; use simple `if/else` where clearer.
...
- **Result Pattern**: Use the `result` library (`from result import Result, Ok, Err`) with `Result[T, E]` typing (Generic over success/error). Wrap expected errors as `Err(error)` and return `Ok(value)` for success.
